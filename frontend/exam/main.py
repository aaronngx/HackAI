...
# Monitor resolution for screen coordinate mapping
...
# Cached iris pixel positions and cover radius (updated each frame face is visible)
cached_l_iris_px = None   # (x, y) pixel of left iris
cached_r_iris_px = None   # (x, y) pixel of right iris
cached_cover_radius = 40  # pixels — updated from face scale
...
while cap.isOpened():
    ...
    if hand_results.multi_hand_landmarks:
        ...
        # Draw hand skeleton on the main frame
        ...
        # Determine pointing direction using index finger:
        # landmark 5 = INDEX_FINGER_MCP (base), landmark 8 = INDEX_FINGER_TIP
        mcp = hand_lm.landmark[5]
        tip = hand_lm.landmark[8]

        # Check index finger is extended (tip further from wrist than MCP)
        wrist = hand_lm.landmark[0]
        tip_dist = math.hypot(tip.x - wrist.x, tip.y - wrist.y)
        mcp_dist = math.hypot(mcp.x - wrist.x, mcp.y - wrist.y)

        if tip_dist > mcp_dist * 1.2:  # finger is extended
            dx = tip.x - mcp.x
            dy = tip.y - mcp.y  # y increases downward in image coords

            if abs(dx) >= abs(dy):
                hand_direction = "RIGHT" if dx > 0 else "LEFT"
            else:
                hand_direction = "DOWN" if dy > 0 else "UP"

        # Draw direction label
        if hand_direction:
            tip_px = (int(tip.x * w), int(tip.y * h))
            dir_colors = {"UP": (0, 255, 0), "DOWN": (0, 100, 255),
                          "LEFT": (255, 200, 0), "RIGHT": (0, 200, 255)}
            cv2.putText(frame, hand_direction, (tip_px[0] - 30, tip_px[1] - 20),
                        cv2.FONT_HERSHEY_SIMPLEX, 1.2, dir_colors[hand_direction], 3)

    # --- Eye coverage check ---
    # Strategy: cache exact iris pixel positions; count how many hand landmarks
    # fall within cover_radius of each iris. Requires dense cluster of landmarks
    # DIRECTLY over each iris — spread/alternating fingers have no landmarks there.
    # MIN_COVER_LM landmarks must be within radius for each eye.
    MIN_COVER_LM = 3  # minimum hand landmarks within iris radius to count as covered

    if results.multi_face_landmarks:
        _lm = results.multi_face_landmarks[0].landmark
        # Iris centers (landmarks 468 = left, 473 = right)
        cached_l_iris_px = (int(_lm[468].x * w), int(_lm[468].y * h))
        cached_r_iris_px = (int(_lm[473].x * w), int(_lm[473].y * h))
        # Cover radius = distance between the two iris centers * 0.6
        d = math.hypot(cached_r_iris_px[0] - cached_l_iris_px[0],
                       cached_r_iris_px[1] - cached_l_iris_px[1])
        cached_cover_radius = max(30, int(d * 0.6))

    left_covered = right_covered = False
    if cached_l_iris_px and cached_r_iris_px and hand_results.multi_hand_landmarks:
        h_lm = hand_results.multi_hand_landmarks[0]
        lx, ly = cached_l_iris_px
        rx, ry = cached_r_iris_px
        r = cached_cover_radius
        l_count = r_count = 0
        for lmk in h_lm.landmark:
            hx, hy = int(lmk.x * w), int(lmk.y * h)
            if math.hypot(hx - lx, hy - ly) <= r:
                l_count += 1
            if math.hypot(hx - rx, hy - ry) <= r:
                r_count += 1
        # --- Count spread (extended) fingers using 3D landmarks ---
        # A finger is "spread/extended" when its tip is further from the wrist
        # than its MCP joint in 3D space (x, y, z all included).
        def lm_xyz(lm, i):
            return np.array([lm.landmark[i].x, lm.landmark[i].y, lm.landmark[i].z])

        def lm_xy(lm, i):
            return np.array([lm.landmark[i].x, lm.landmark[i].y])

        wrist = lm_xyz(h_lm, 0)
        # (finger_name, tip_idx, mcp_idx)
        FINGERS = [("Thumb", 4, 2), ("Index", 8, 5),
                   ("Middle", 12, 9), ("Ring", 16, 13), ("Pinky", 20, 17)]
        EXTEND_RATIO = 1.3  # tip must be 1.3x further from wrist than MCP

        spread_fingers = []
        for name, tip_i, mcp_i in FINGERS:
            tip_dist = np.linalg.norm(lm_xyz(h_lm, tip_i) - wrist)
            mcp_dist = np.linalg.norm(lm_xyz(h_lm, mcp_i) - wrist)
            if mcp_dist > 1e-6 and tip_dist / mcp_dist > EXTEND_RATIO:
                spread_fingers.append(name)

        num_spread = len(spread_fingers)
        fingers_together = num_spread <= 1  # ≤1 spread finger = hand is closed/together

        left_covered  = l_count >= MIN_COVER_LM and fingers_together
        right_covered = r_count >= MIN_COVER_LM and fingers_together

    eyes_covered = left_covered and right_covered

    # Draw iris coverage circles
    if cached_l_iris_px:
        color = (0, 0, 255) if left_covered else (0, 255, 200)
        cv2.circle(frame, cached_l_iris_px, cached_cover_radius, color, 2)
    if cached_r_iris_px:
        color = (0, 0, 255) if right_covered else (0, 255, 200)
        cv2.circle(frame, cached_r_iris_px, cached_cover_radius, color, 2)

    # ── Bottom HUD (35px per row from bottom up) ──────────────────────────
    l_text  = "COVER L: COVERED" if left_covered  else "COVER L: OPEN"
    r_text  = "COVER R: COVERED" if right_covered else "COVER R: OPEN"
    l_color = (0, 0, 255) if left_covered  else (0, 220, 0)
    r_color = (0, 0, 255) if right_covered else (0, 220, 0)
    cv2.putText(frame, l_text, (10, h - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, l_color, 2)
    cv2.putText(frame, r_text, (10, h - 15), cv2.FONT_HERSHEY_SIMPLEX, 0.8, r_color, 2)

    # Finger spread status row
    if hand_results.multi_hand_landmarks:
        if fingers_together:
            fc_text  = f"SPREAD: 0 fingers"
            fc_color = (0, 220, 0)
        else:
            names    = ", ".join(spread_fingers)
            fc_text  = f"SPREAD: {num_spread} ({names}) CHEAT!"
            fc_color = (0, 100, 255)
        cv2.putText(frame, fc_text, (10, h - 85), cv2.FONT_HERSHEY_SIMPLEX, 0.75, fc_color, 2)

    if results.multi_face_landmarks:
        face_landmarks = results.multi_face_landmarks[0].landmark

        # --- Face distance estimation ---
        # Using average adult IPD = 63mm and pinhole camera model
        REAL_IPD_MM = 63.0
        ASSUMED_FOV_DEG = 60.0
        focal_px = w / (2.0 * math.tan(math.radians(ASSUMED_FOV_DEG / 2.0)))
        l_iris = face_landmarks[468]
        r_iris = face_landmarks[473]
        pixel_ipd = math.hypot((r_iris.x - l_iris.x) * w, (r_iris.y - l_iris.y) * h)
        if pixel_ipd > 1.0:
            face_dist_cm = (REAL_IPD_MM * focal_px) / (pixel_ipd * 10.0)
            dist_text = f"Distance: {face_dist_cm:.1f} cm"
        else:
            dist_text = "Distance: --"
        cv2.putText(frame, dist_text, (10, 60),
        ...
        if left_sphere_locked and right_sphere_locked:
            ...
            # Show screen gaze position
            screen_text = f"Screen: ({screen_x}, {screen_y})"
            (tw, _), _ = cv2.getTextSize(screen_text, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)
            cv2.putText(frame, screen_text, ((w - tw) // 2, 30),
