  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!token) {
        ...
        setNarrationLines([]);
        setInputError("Please log in to view your latest report.");
      ...
      // Source of truth: latest DB record for the current logged-in user.
      const resolvedInput = history.length > 0 ? mapHistoryToInput(history[0]) : null;
      ...
      if (!resolvedInput) {
        ...
        setNarrationLines([]);
        setInputError(
          "No eye diagnostic result found for this account yet. Run Eye Disease Detect while logged in."
        );
        setPageLoading(false);
        return;
      }
      ...
      const r = await generateDetectionReport(resolvedInput, history);
      if (!isMounted) return;
      setReport(r);
